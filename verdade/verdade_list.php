<?php
	include_once ('../verdade/verdade.php');
?>
...
<link rel="stylesheet" href="../livraria/style.css"/>
	...
	// ki o cabaxalho enviado pelo arkivo HTML seja mesmo que HTTP do servidor
			...
			print "<br/ >";
			$cabecalho_verd=verdade_cabe();
			$link_verd=verdade_links();
			
			
          do{
          	
			// imprimir os links para acesso as noticias
			echo "<li><a class=\"cont2\" style=\"white-space: normal; text-align: justify\" href="."/verdade/verdade_cont.php?name=".current($link_verd)," data-prefetch=\"true\">",current($cabecalho_verd),"</a>","<li>";
				
				//echo current($cabecalho2), ".", current($link2), "<br/>";
          	
          } while( next($cabecalho_verd)&& next($link_verd));
